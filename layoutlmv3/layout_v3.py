...
save_model_path = "/data/experiments/1data/savedmodel/layoutlmv3/"
...
# label2idx
from custon_datasetwithoutpad import idx2label,label2idx
...
class LayoutLMForTokenClassification(nn.Module):
    def __init__(self, output_size=(3, 3),
        ...
        self.layoutlm  = LayoutLMv3ForTokenClassification.from_pretrained('microsoft/layoutlmv3-base', num_labels=self.num_labels)
...
def test(epoch):
    ...
    if f1_score(out_label_list, preds_list) > 0.83:
        save_info = {  # 保存的信息
    ...
    saveLog(epoch=epoch,loss=eval_loss,f1_score="%.4f" % f1_score(out_label_list, preds_list))



def saveLog(epoch,loss,f1_score):
  result_dir = "/data/experiments/1data/result/v3"
  train_log_filename = "layoutLMV3_train_log.txt"
  train_log_filepath = os.path.join(result_dir, train_log_filename)
  train_log_txt_formatter = "{time_str} [Epoch] {epoch:03d} [Loss] {loss_str} [f1] {f1}\n"
