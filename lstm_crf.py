...
class BERT_LSTM_CRF(nn.Module):
    ...
    bert_lstm_crf model
    """
    def __init__(self, tagset_size, embedding_dim, hidden_dim, rnn_layers, dropout_ratio, dropout1, use_cuda=False):
    ...
    def rand_init_hidden(self, batch_size):
        ...
        """
        return  torch.randn(2 * self.rnn_layers, batch_size, self.hidden_dim,requires_grad=True),torch.randn(2 * self.rnn_layers, batch_size, self.hidden_dim,requires_grad=True)
    ...
    def forward(self, sentence, attention_mask=None):
        ...
        lstm_out, hidden = self.lstm(sentence, (hidden[0].to(config.device),hidden[1].to(config.device)))
