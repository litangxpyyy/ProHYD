...
class GraphLearningLayer(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Tensor, box_num: Tensor = None):
        ...
        # (B, N, D)
        x_hat = self.projection(x)  #[5,10,128]
        _, _, learning_dim = x_hat.shape #[128]

        # (B, N, N, learning_dim)
        x_i = x_hat.unsqueeze(2).expand(B, N, N, learning_dim)  #[5,10,10,128]
        x_j = x_hat.unsqueeze(1).expand(B, N, N, learning_dim) # #[]5,10,10,128]
        # (B, N, N, learning_dim)
        distance = torch.abs(x_i - x_j) #[5,10,10,128]
...
class GLCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.alpha_transform = nn.Linear(5, in_dim, bias=False)
    ...
    def forward(self, x: Tensor, rel_features: Tensor, adj: Tensor, box_num: Tensor, **kwargs):
        ...
        for i, gcn_layer in enumerate(self.gcn):
            ...
            # adj = soft_adj
            # x = self.dropout(x)
        return x, soft_adj, gl_loss,alpha
